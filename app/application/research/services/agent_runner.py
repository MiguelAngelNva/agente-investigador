...
async def run_team(query: str, session_id: str, tracer: AgentTracer, on_progress=None) -> str:
    ...
    existing = await _session_service.get_session(
        app_name=settings.adk_app_name, user_id=settings.adk_user_id, session_id=session_id
    )
    if existing is None:
    ...
    runner = Runner(agent=team, app_name=settings.adk_app_name, session_service=_session_service)
    ...
    async for event in runner.run_async(
        user_id=settings.adk_user_id, session_id=session_id, new_message=message
    ):
        author = event.author
